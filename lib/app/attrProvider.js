define([], function () {
    return function (app) {
        ...
        app.createProvider('listenerBinder');

        var eventPattern = /^(\S+)\s*(.*)$/;

        app.listenerBinder.add('default', {
          exec: function(view, name, listener){
              ...
              view.listenTo(target, event, listener);
          }
        });

        app.listenerBinder.add('ui', {
            exec: function(view, name, listener){
                var match = eventPattern.exec(name);
                var event = match[1];
                var target = match[2];
        ...
        app.listenerBinder.add('bus', {
            exec: function(view, name, listener){
                view.sub(name, listener);
            }
        });
