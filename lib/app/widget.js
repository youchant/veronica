    ...
    return function (app) {
        ...
        function executeWidget(mainFunc, options, dfd) {
            ...
            if (pageName && app.isCurrPage && !app.isCurrPage(pageName)) {
                ...
                if (_.isUndefined(funcResult)) {
                    ...
                    //  如果是延迟对象
                    var resultIsDefferd = funcResult.done && funcResult.fail;
                    widgetDfd = resultIsDefferd ? funcResult : $.Deferred();

                    widgetDfd.done(function (callback) {
                        var widgetObj = _.isFunction(callback) ? callback(options) : callback;
                    ...
                    !resultIsDefferd && widgetDfd.resolve(funcResult);
                }
        ...
        app.widget.start = function (list, callback, page) {
            ...
            _(list).each(function (widgetConfig) {
                ...
                // 如果老页面和新页面都存在同样的部件，则发布一个消息，用于可能的选项设置
                var same = _.find(app.widget.oldWidgetList, function (oldConfig) {
                    return oldConfig.name === widgetConfig.name &&
                    oldConfig.options.host === widgetConfig.options.host &&
                        oldConfig.options._tag === widgetConfig.options._tag;
                });

                // 仅当前host没有相同的部件时或者有相同部件但tag不一样时，才加载
                if (widgetName !== 'empty' && (noWidget || !same)) {
        ...
        app.widget.wait = function (dependency, sandbox, callback) {
            var promises = [];
            _(dependency).each(function (name) {
                var widgetSandbox = app.sandboxes.getByName(name);
